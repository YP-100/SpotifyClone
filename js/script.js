...
async function getSongs(gfolder) {
    currefolder = gfolder;
    try {
        let songs = await fetch(`http://127.0.0.1:8080/${currefolder}`);
...
const playMusic = (mp3) => {
    ...
    let prefix =   `http://127.0.0.1:8080/${currefolder}/` ;
...
async function main() {
    try {
        ...
        Array.from(document.getElementsByClassName("pcardcontainer")).forEach(elem => {
            elem.addEventListener("click", async event => {
                ...
                if (originalName) {
                    playMusic(`http://127.0.0.1:8080/${currefolder}/${originalName}`);
                }
                    });
                });
            });
    ...
    // Define the prefix to remove
    let prefix =   `ttp://127.0.0.1:8080/${currefolder}/`;
    ...
    if (indexof != 0) {
        ...
        playMusic(`http://127.0.0.1:8080/${currefolder}/${sname[indexof - 1]}`);
    }
    })
